		...
		if (b != null && (b.getType() == Material.SIGN_POST || b.getType() == Material.WALL_SIGN))
		{
			...
			if (sign.getLine(0).contains("[iProfessions]"))
			{
				...
				if (data[0].equalsIgnoreCase("learn"))
				{
					...
					if (p != null)
					{
						if (!plug.getPermissionHandler().has(player, "sign.learn." + p.getTag(), true)
								  && !plug.getPermissionHandler().has(player, "sign.learn", true))
						{
					...
					if (p != null)
					{
						if (!plug.getPermissionHandler().has(player, "sign.forget." + p.getTag(), true)
								  && !plug.getPermissionHandler().has(player, "sign.forget", true))
						{
